...
for await (const page of pages) for (const model of page.result.data) {
  ...
  if (process.argv.includes("--critical-only") && !freePool.has(model.id)) continue;
...
// Prioritize the free managed pool that users see first; check other eligible models after it.
models.sort((a, b) => Number(freePool.has(b.id)) - Number(freePool.has(a.id)));
    ...
    try {
      for (const prompt of ["Reply with a short greeting.", "Return one complete minimal HTML document for a dog website, under 100 words. No explanation."]) {
        ...
        const result = streamText({
          ...
          prompt,
          maxOutputTokens: Math.min(freePool.has(model.id) ? 8192 : 2048, model.topProvider.maxCompletionTokens ?? 2048),
          maxRetries: 0,
          abortSignal: AbortSignal.timeout(45000),
          timeout: { chunkMs: 20000 },
