...
const freePool = new Set(["inclusionai/ling-3.0-flash:free", "nvidia/nemotron-3.5-lightning:free", "nvidia/nemotron-3-ultra-550b-a55b:free"]);
...
async function worker() {
  while (index < models.length) {
    ...
    try {
      for (const prompt of ["Reply with a short greeting.", "Return one complete minimal HTML document for a dog website, under 100 words. No explanation."]) {
        ...
        const result = streamText({
          model: provider.chat(model.id, { usage: { include: true }, ...(model.supportedParameters.includes("reasoning") ? { reasoning: { effort: "low" } } : {}) }),
          prompt, maxOutputTokens: Math.min(2048, model.topProvider.maxCompletionTokens ?? 2048),
    ...
    console.log("[model-smoke]", model.id, report.models[model.id].status);
  }
}
