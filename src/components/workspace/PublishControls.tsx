...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[180] flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm sm:items-center sm:p-4">
      <button
        type="button"
        ...
        aria-label="Close"
      />
      <div className="relative z-10 max-h-[94dvh] w-full max-w-xl overflow-hidden rounded-t-[28px] border border-zinc-800 bg-zinc-950 shadow-2xl shadow-black sm:max-h-[90vh] sm:rounded-2xl">
        ...
        <div className="max-h-[calc(94dvh-64px)] overflow-y-auto p-5 sm:max-h-[calc(90vh-64px)]">
          {children}
        </div>
      </div>
    </div>
        ...
        <TextField
          ...
          helper={
            <TokenHelp
              ...
              note="You can also save this under Developer Connections."
            />
          }
        />
...
function DeployPublishModal({
  ...
  const [activeProvider, setActiveProvider] = useState<
    "cryzo" | "vercel" | "netlify"
  >("cryzo");
  const [vercelToken, setVercelToken] = useState(() =>
    readDeveloperToken("vercel"),
  );
  const [netlifyToken, setNetlifyToken] = useState(() =>
    readDeveloperToken("netlify"),
  );
  ...
  const [desiredCryzoUrl, setDesiredCryzoUrl] = useState(
    `https://${defaultName}.cryzo.me`,
  );
  const [status, setStatus] = useState<PublishStatus>({ type: "idle" });
  ...
  const [mobileReport, setMobileReport] = useState<StoreReadinessReportData | null>(
    null,
  );
  ...
  const [androidTrack, setAndroidTrack] = useState<
  ...
  const deployCryzo = async () => {
    if (!authToken) {
      return setStatus({
        type: "error",
        message: "Your Cryzo session is still loading. Try again.",
      });
    }
    setStatus({
      type: "loading",
      message: existingTarget
        ? "Publishing a new version and syncing Cryzo Cloud..."
        : "Creating your Cryzo app and Cloud backend...",
    });
    ...
    const response = await fetch("/api/publish/cryzo", {
      ...
      body: JSON.stringify({
        ...
        supabase: supabase
          ? { url: supabase.url, publicKey: supabase.publicKey }
          : null,
      }),
    });
    const data = await response.json();
    if (!response.ok) {
      if (data.code === "CRYZO_HOSTING_NOT_CONFIGURED") {
        setHostingConfigured(false);
      }
      return setStatus({
        type: "error",
        message: data.error || "Cryzo Hosting publish failed",
      });
    }
    ...
    setDesiredCryzoUrl(
      data.desiredUrl || `https://${data.slug}.${hostingDomain}`,
    );
    setExistingTarget({
      ...
      customDomain: data.subdomainAssigned
        ? `${data.slug}.${hostingDomain}`
        : undefined,
    });
    ...
    if (data.subdomainAssigned) {
      setStatus({
        type: "success",
        message: "Published with Cryzo Cloud.",
        url: data.url,
      });
    } else {
  ...
  const deployVercel = async () => {
    setStatus({
      type: "loading",
      message: "Creating deployment in your Vercel account...",
    });
    ...
    setStatus({
      type: "success",
      message: "Deployment started in your Vercel account.",
      url: data.url,
    });
  };
  ...
  const deployNetlify = async () => {
    ...
    try {
      ...
      const siteId =
        localStorage.getItem(`cryzo:netlify-site:${conversationId}`) || undefined;
  ...
  const mobileRequest = async (
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.error || `Mobile ${operation} failed`);
    }
    return data;
  };
    ...
    try {
      ...
      if (!data.ready) {
        ...
        return setMobileStatus({
          ...
          message:
            (data.issues || []).join(" ") ||
            "App has blocking store-readiness issues.",
        });
      }
      ...
      setMobileStatus({
        ...
        message: (data.warnings || []).length
          ? "Scan complete. No blocking issues; review warnings below."
  ...
  const buildMobile = async () => {
    if (!managedBuildIncluded) {
      return setMobileStatus({
        type: "error",
        message:
          "Managed EAS builds are included on Builder and above. The readiness scan and source stay free.",
      });
    }
    setMobileStatus({
      type: "loading",
      message: `Starting ${mobilePlatform === "ios" ? "iOS" : "Android"} EAS build...`,
    });
    try {
      ...
      setMobileStatus({
        type: "success",
        message: `EAS build ${data.status || "queued"}.`,
        url: data.buildUrl,
      });
    } catch (error) {
      setMobileStatus({
  ...
  const refreshMobile = async () => {
    ...
    setMobileStatus({
      type: "loading",
      message: "Refreshing EAS build status...",
    });
    try {
      const data = await mobileRequest("status", {
      ...
      setMobileStatus({
        type: "success",
        message: `Build status: ${data.status}.`,
        url: data.artifactUrl || data.buildUrl,
      });
    } catch (error) {
      setMobileStatus({
        type: "error",
  ...
  const submitMobile = async () => {
    ...
    if (!managedSubmissionIncluded) {
      return setMobileStatus({
        type: "error",
        message:
          "One-click store submission is included on Builder and above. You can still submit the exported project yourself.",
      });
    }
    setMobileStatus({
      type: "loading",
      message: `Submitting ${mobileBuild.platform === "ios" ? "to App Store Connect" : "to Google Play"}...`,
    });
    try {
    ...
    <Modal title="Publish Your App" onClose={onClose}>
      <div className="space-y-5">
        <div className="flex border-b border-zinc-800">
          ...
          />
          <ModeTab
            active={publishMode === "mobile"}
            label="Mobile app"
            icon={<Smartphone size={15} />}
            onClick={() => {
              setPublishMode("mobile");
              setMobileStatus({ type: "idle" });
            }}
          />
        </div>
              ...
              <div className="space-y-4">
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-3">
                    ...
                    <div className="mt-1 text-sm text-white">
                      {brandingRequired
                        ? "Built with Cryzo badge"
                        : "Branding removed"}
                    </div>
                    <div className="mt-1 text-xs leading-5 text-zinc-500">
                      {brandingRequired
                        ? "Free managed deployments include a subtle badge. Exported and BYO deployments stay clean."
                        : `${plan} includes unbranded managed hosting.`}
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-3">
                    ...
                    <div className="mt-1 text-sm text-white">
                      {cloudBackend ? "Database + auth active" : "Included on every plan"}
                    </div>
                    <div className="mt-1 text-xs leading-5 text-zinc-500">
                      Database, app users and authentication are built in. Managed runtime operations use your integration-credit allowance.
                    </div>
                    <a
                      href={`/chat/cloud?conversationId=${encodeURIComponent(conversationId)}`}
                      className="mt-2 inline-flex items-center gap-1 text-xs text-blue-300 hover:text-blue-200"
                    >
                      Open Cloud <ExternalLink size={11} />
                    </a>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex h-10 items-center overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900">
                    <span className="shrink-0 border-r border-zinc-800 px-3 text-xs text-zinc-500">
                      https://
                    </span>
                    <input
                      value={cryzoSlug}
                      onChange={(event) =>
                        setCryzoSlug(
                          sanitizeName(event.target.value, "app").slice(0, 48),
                        )
                      }
                      className="min-w-0 flex-1 bg-transparent px-2 text-base text-white outline-none sm:text-sm"
                      aria-label="Cryzo URL slug"
                    />
                    <span className="shrink-0 px-3 text-xs text-zinc-500">
                      .{hostingDomain}
                    </span>
                  </div>
                  <p className="mt-1.5 text-xs text-zinc-500">
                    Built-in Cryzo URLs are included. Renaming a managed Cryzo URL is Starter+; DIY Vercel domains remain yours.
                  </p>
                </div>
                ...
                {existingTarget?.url && (
                  <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
                    ...
                    {existingTarget.url !== desiredCryzoUrl && (
                      <div className="mt-2 text-xs text-zinc-500">
                        Target Cryzo URL: {desiredCryzoUrl}
                      </div>
                    )}
                  </div>
                )}
                ...
                {hostingConfigured === false && (
                  <p className="rounded-lg border border-amber-900/70 bg-amber-950/30 px-3 py-2 text-xs leading-5 text-amber-300">
                    Managed hosting needs CRYZO_VERCEL_TOKEN on the Cryzo server. Your Vercel tab remains available.
                  </p>
                )}
                ...
                <button
                  ...
                  className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-40"
                >
                  {isLoading ? (
                    <Loader2 size={15} className="animate-spin" />
                  ) : (
            ...
            {activeProvider === "vercel" && (
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-white px-4 text-sm font-medium text-black disabled:opacity-40"
                >
                  {isLoading ? (
                    <Loader2 size={15} className="animate-spin" />
                  ) : (
          ...
          <div className="space-y-5">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-zinc-500">
                The readiness scan is free. Cryzo-managed EAS builds and one-click store submission are Builder+ conveniences; source export and manual submission remain available.
              </p>
            </div>

            <div className="grid grid-cols-2 gap-2">
              <ProviderButton
                active={mobilePlatform === "ios"}
                label="iOS"
                icon={<Store size={15} />}
                onClick={() => {
            ...
            <TextField
              label="Published web URL"
              value={mobileWebUrl}
              onChange={setMobileWebUrl}
              placeholder="https://your-app.cryzo.me"
            />
            <TextField
              label="App name"
              value={mobileAppName}
              onChange={setMobileAppName}
            />
            <TextField
              label={
                mobilePlatform === "ios"
              ...
              onChange={setMobileIdentifier}
              placeholder="com.yourcompany.app"
            />
            <TextField
              label="Expo account / organization"
              value={expoAccount}
              onChange={setExpoAccount}
              placeholder="your-expo-account"
            />
            <TextField
              label="Expo access token"
              value={expoToken}
              onChange={setExpoToken}
              type="password"
              placeholder="Expo access token"
              helper={
                <TokenHelp
                  href="https://expo.dev/accounts/[account]/settings/access-tokens"
                  note="Needed only for Cryzo-managed EAS build/submit. Saved on this device when used."
                />
              }
            />
            ...
            <MobileStep number={1} title="Scan Store Readiness" complete={mobileCheckPassed}>
              <button
                type="button"
                onClick={() => void checkMobile()}
                disabled={mobileLoading}
                className="inline-flex h-9 items-center gap-2 rounded-lg bg-white px-3 text-sm font-medium text-black disabled:opacity-40"
              >
                {mobileLoading ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : (
            ...
            <MobileStep
              number={2}
              title="Build Store Files"
              complete={Boolean(mobileBuild)}
            >
              {!managedBuildIncluded && mobileReport && (
                <PlanGate
                  title="Builder+ managed build"
                  text="Your scan and source are still free. Upgrade only if you want Cryzo to run EAS Build for you."
                />
              )}
              <button
                type="button"
                onClick={() => void buildMobile()}
                disabled={
                  !mobileCheckPassed || mobileLoading || !managedBuildIncluded
                }
                className="inline-flex h-9 items-center gap-2 rounded-lg bg-blue-600 px-3 text-sm font-medium text-white disabled:opacity-40"
              >
                {mobileLoading ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : managedBuildIncluded ? (
                  <Smartphone size={14} />
                ) : (
                  <LockKeyhole size={14} />
                )}
              ...
              {mobileBuild && (
                <button
                  type="button"
                  onClick={() => void refreshMobile()}
                  disabled={mobileLoading}
                  className="ml-2 inline-flex h-9 items-center gap-2 rounded-lg border border-zinc-700 px-3 text-sm text-zinc-200 disabled:opacity-40"
                >
            ...
            <MobileStep number={3} title="Submit Your App" complete={false}>
              <div className="space-y-3">
                {!managedSubmissionIncluded && mobileReport && (
                  <PlanGate
                    title="Builder+ one-click submission"
                    text="You can still submit the generated Expo app manually. Builder+ lets Cryzo handle the EAS submission step."
                  />
                )}
                ...
                {mobileBuild?.platform === "ios" ? (
                  <div className="space-y-3">
                    ...
                    <TokenHelp
                      ...
                      note="Create an App Store Connect API key with the access needed to submit builds."
                    />
                  </div>
                ) : mobileBuild?.platform === "android" ? (
                  <div className="space-y-3">
                    ...
                    <label className="block">
                      ...
                      <select
                        value={androidTrack}
                        onChange={(event) =>
                          setAndroidTrack(event.target.value as typeof androidTrack)
                        }
                    ...
                    <TokenHelp
                      ...
                      note="Use a Google service account that has access to this app in Play Console."
                    />
                  </div>
                ) : (
                  <p className="text-xs text-zinc-500">
                    Build an iOS or Android store file first.
                  </p>
                )}
              ...
              <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-3 text-xs text-zinc-400">
                ...
                <div className="mt-1">
                  ...
                  <span className="text-zinc-200">
                    {mobileBuild.status || "queued"}
                  </span>
                </div>
                {(mobileBuild.artifactUrl || mobileBuild.buildUrl) && (
                  <a
                    href={mobileBuild.artifactUrl || mobileBuild.buildUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-2 inline-flex items-center gap-1 text-blue-300"
                  >
                    Open build <ExternalLink size={12} />
                  </a>
                )}
              </div>
            )}
...
function ModeTab({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex items-center gap-2 border-b-2 px-5 py-3 text-sm",
        active
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-10 w-full rounded-lg border border-zinc-800 bg-zinc-900 px-3 text-base text-white outline-none transition-colors placeholder:text-zinc-600 focus:border-zinc-600 sm:text-sm"
      />
  ...
  return (
    <label className="block">
      ...
      <span className="mt-2 flex items-center justify-between gap-2 text-xs text-zinc-500">
        <span>
          {value.trim() ? "Credential loaded" : "Paste it above or choose a file."}
        </span>
...
function TokenHelp({ href, note }: { href: string; note?: string }) {
  return (
    <span className="inline-flex flex-wrap items-center gap-2 text-xs text-zinc-500">
      ...
      >
        Get your token <ExternalLink size={12} />
      </a>
      {note && (
        <>
